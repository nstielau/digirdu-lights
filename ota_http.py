...
class DeviceHTTP:
    def __init__(self, pool, device_id, token, api_base):
        if (
            len(token) != 64
        ...
        suffix = ".firebaseapp.com/device-api/v1"
        if not api_base.startswith("https://") or not api_base.endswith(suffix):
            raise ValueError("invalid_ota_endpoint")
        project = api_base[8:-len(suffix)]
        if not project or any(c not in "abcdefghijklmnopqrstuvwxyz0123456789-" for c in project):
            raise ValueError("invalid_ota_endpoint")
        self.host = project + ".firebaseapp.com"
        self.pool, self.device_id, self.token = pool, device_id, token

    def request(self, route, service, consume, limit=4096, body=None):
        from ota_manifest import APP_FILES, is_digest
        pieces = route.split("/")
        if route not in ("check-in", "report") and not (
            len(pieces) == 3 and pieces[0] == "artifacts" and is_digest(pieces[1])
            and pieces[2] in APP_FILES
        ):
